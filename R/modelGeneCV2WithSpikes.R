...
#' A trend is fitted to the variance against the mean for spike-in transcripts using \code{\link{fitTrendCV2}}.
...
#'
#' This function can be considered the same as \code{\link{modelGeneCV2}}, with the only theoretical difference being that the trend is fitted on spike-in variances rather than using the means and variances of endogenous genes.
#' Both methods compute variances and means on the log-expression values;
#' both methods use \code{\link{fitTrendCV2}} to fit the mean-variance trend; 
#' and both methods allow blocking or regression of uninteresting factors of variation.
#' 
#' \code{modelGeneCV2WithSpikes} starts from a count matrix (for both genes and spike-ins) and requires size factors and a pseudo-count specification to compute the log-expression values.
...
#' \item{\code{total}:}{CV2iance of the normalized log-expression per gene.}
