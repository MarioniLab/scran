...
#' @param cluster A vector of factor of length equal to \code{ncol(x)},
...
#' we often have clusters made of cells from different samples of different experimental conditions.
...
#' this function loops over the clusters and uses \pkg{edgeR} to detect DE genes between conditions.
#' The DE analysis for each cluster is largely the same as a standard analysis for bulk RNA-seq data,
#' using \code{design} and \code{coef} or \code{contrast} as described in the \pkg{edgeR} user's guide.
#' The analysis for each cluster is independent, i.e., the GLM fitting is performed separately,
#' to minimize problems due to differences in abundance and variance between clusters.
#'
...
#'
#' For each cluster, abundance filtering is performed using \code{\link{filterByExpr}} prior to further analysis.
#' Genes that are filtered out will still show up in the DataFrame for that cluster, but with all statistics set to \code{NA}.
#' Note that a different set of genes may be filtered out for each cluster,
#' which is largely to be expected if their abundances change between clusters.
