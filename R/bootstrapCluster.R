...
#' 
#' @export
bootstrapCluster <- function(x, FUN, clusters=NULL, transposed=FALSE, iterations=20, ..., compare=NULL, summarize=FALSE) {
    if (is.null(clusters)) {
        clusters <- FUN(x, ...)
    }
    ...
    if (use.coassign <- is.null(compare)) {
        compare <- coassignProb
    }
    ...
    if (!use.coassign || !summarize) {
        averaged 
    } else {
        .summarize_coassign(averaged)
    }
}
