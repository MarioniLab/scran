...
example.sce <- SingleCellExperiment::SingleCellExperiment(list(counts=combined))
example.sce <- scater::normalize(example.sce)
SingleCellExperiment::isSpike(example.sce, "Spike") <- ngenes + seq_len(nspikes)
