...
test_that("quickCluster works in the simple case", {
    emp.clusters <- quickCluster(dummy, use.ranks=FALSE, BSPARAM=ExactParam())
    expect_true(length(unique(paste0(known.clusters, emp.clusters)))==3L)
    emp.clusters <- quickCluster(dummy, d=20, use.ranks=FALSE, BSPARAM=ExactParam())
    expect_true(length(unique(paste0(known.clusters, emp.clusters)))==3L)

    # Behaves after rows are shuffled.
    shuffled <- sample(nrow(dummy))
    shuf.clusters <- quickCluster(dummy, subset.row=shuffled, use.ranks=FALSE, BSPARAM=ExactParam())
    expect_true(length(unique(paste0(shuf.clusters, emp.clusters)))==3L)

    # Behaves with use.ranks=TRUE.
    emp.clusters <- quickCluster(dummy, use.ranks=TRUE, BSPARAM=ExactParam())
    expect_true(length(unique(paste0(known.clusters, emp.clusters)))==3L)
    emp.clusters <- quickCluster(dummy, use.ranks=TRUE, d=20, BSPARAM=ExactParam())
    expect_true(length(unique(paste0(known.clusters, emp.clusters)))==3L)
...
test_that("use.ranks=TRUE generates the correct DeferredMatrix", {
    ...
    bspar <- ExactParam(deferred=TRUE)
...
test_that("quickCluster functions correctly with subsetting", {
    ...
    obs <- quickCluster(mat, min.size=50, subset.row=subset.row, use.ranks=FALSE, BSPARAM=ExactParam())
    ...
    obs <- quickCluster(mat, min.size=50, subset.row=subset.row, use.ranks=TRUE, method="hclust", BSPARAM=ExactParam())
    expect_identical(obs, quickCluster(mat[subset.row,], min.size=50, use.ranks=TRUE, method="hclust", BSPARAM=ExactParam()))
    expect_false(identical(quickCluster(mat, min.size=50, use.ranks=TRUE, method="hclust", BSPARAM=ExactParam()), obs))

    # Handles the mean, but only when use.ranks=TRUE.
    obs <- quickCluster(mat, min.size=50, min.mean=5, use.ranks=TRUE, BSPARAM=ExactParam())
    expect_identical(obs, quickCluster(mat, min.size=50, use.ranks=TRUE, 
        subset.row=scater::calcAverage(mat) >= 5, BSPARAM=ExactParam()))

    # 'min.mean' should have no effect when use.ranks=FALSE.
    obs <- quickCluster(mat, min.size=50, min.mean=5, use.ranks=FALSE, BSPARAM=ExactParam())
    expect_identical(obs, quickCluster(mat, min.size=50, min.mean=1, use.ranks=FALSE, BSPARAM=ExactParam())) 
})
...
test_that("quickCluster functions correctly with blocking", {
    ...
    obs <- quickCluster(mat, min.size=10, block=block, use.ranks=FALSE, BSPARAM=ExactParam())

    collected <- numeric(ncol(mat))
    last <- 0L
    for (x in sort(unique(block))) {
        chosen <- block==x
        current <- quickCluster(mat[,chosen], min.size=10, use.ranks=FALSE, BSPARAM=ExactParam())
    ...
    ref <- quickCluster(mat, min.size=10, block=NULL, use.ranks=FALSE, BSPARAM=ExactParam())
    obs <- quickCluster(mat, min.size=10, block=integer(ncol(mat)), use.ranks=FALSE, BSPARAM=ExactParam())
    expect_identical(ref, obs)

    # Should avoid problems with multiple BPPARAM specifications.
    ref <- quickCluster(mat, min.size=10, block=block, method="igraph", use.ranks=FALSE, BSPARAM=ExactParam())
    obs <- quickCluster(mat, min.size=10, block=block, method="igraph", use.ranks=FALSE, 
        block.BPPARAM=safeBPParam(2), BSPARAM=ExactParam())
...
test_that("quickCluster's calls to min.size in dynamic tree cut are respected", {
    ...
    out <- quickCluster(dummy, min.size=0, method="hclust", use.ranks=FALSE, BSPARAM=ExactParam())
    ...
    expect_warning(forced <- quickCluster(dummy[,keep], method="hclust", use.ranks=FALSE, min.size=100, BSPARAM=ExactParam()), 
...
test_that("quickCluster with igraph works correctly", {
    ...
    obs <- quickCluster(mat, min.size=0, method="igraph", k=k, d=50, use.ranks=FALSE, BSPARAM=ExactParam())
    ...
    obs <- quickCluster(mat, min.size=0, method="igraph", k=k, d=50, graph.fun=igraph::cluster_fast_greedy, 
        use.ranks=FALSE, BSPARAM=ExactParam())
    out <- igraph::cluster_fast_greedy(snn)
    expect_identical(factor(out$membership), obs)

    # Checking that 'd' is respected, along with other arguments that are passed along.
    obs <- quickCluster(mat, min.size=0, method="igraph", d=20, k=15, use.ranks=FALSE, BSPARAM=ExactParam())
    snn <- buildSNNGraph(ref, d=20, k=15)
    out <- igraph::cluster_walktrap(snn)
    expect_identical(factor(out$membership), obs)

    obs <- quickCluster(mat, min.size=0, method="igraph", d=NA, k=5, use.ranks=FALSE, BSPARAM=ExactParam())
...
test_that("quickCluster with igraph merging works correctly", {
    k <- 10
    mat <- matrix(rnorm(200000, mean=20), nrow=400)
    obs <- quickCluster(mat, min.size=0, method="igraph", k=k, use.ranks=FALSE, BSPARAM=ExactParam())

    min.size <- 100
    expect_false(all(table(obs) >= min.size))
    obs2 <- quickCluster(mat, min.size=min.size, method="igraph", k=k, use.ranks=FALSE, BSPARAM=ExactParam())
...
test_that("quickCluster with igraph on ranks works correctly", {
    ...
    # Testing igraph mode.
    obs <- quickCluster(mat, min.size=0, method="igraph", k=k, use.ranks=TRUE, BSPARAM=ExactParam())
    ...
    obs <- quickCluster(mat, min.size=0, method="igraph", k=10, d=20, use.ranks=TRUE, BSPARAM=ExactParam())
...
test_that("quickCluster fails on silly inputs", {
    ...
    expect_error(quickCluster(dummy[,0], use.ranks=TRUE, BSPARAM=ExactParam()), "a dimension is zero")
...
test_that("quickCluster works on SingleCellExperiment objects", {
    ...
    emp.clusters <- quickCluster(X, use.ranks=FALSE, BSPARAM=ExactParam())
    expect_identical(emp.clusters, quickCluster(counts(X), use.ranks=FALSE, BSPARAM=ExactParam()))

    # Checking correct interplay between spike-ins and subset.row.
    isSpike(X, "ERCC") <- 1:20
    expect_identical(quickCluster(X, use.ranks=FALSE, BSPARAM=ExactParam()), 
        quickCluster(counts(X)[-(1:20),], use.ranks=FALSE, BSPARAM=ExactParam()))

    subset.row <- 1:25*2
    set.seed(0) # set.seed() for consistent tie handling by BiocNeighbors.
    clust1 <- quickCluster(X, use.ranks=FALSE, subset.row=subset.row, BSPARAM=ExactParam()) 
    set.seed(0)
    clust2 <- quickCluster(counts(X)[setdiff(subset.row, 1:20),], use.ranks=FALSE, BSPARAM=ExactParam())
    expect_identical(clust1, clust2)

    expect_identical(quickCluster(X, subset.row=subset.row, get.spikes=TRUE, use.ranks=FALSE, BSPARAM=ExactParam()),
...
test_that("quickCluster works on alternative matrices", {
    ...
    out <- quickCluster(sparse, min.mean=0, use.ranks=TRUE, BSPARAM=ExactParam())
    ...
    ref <- quickCluster(as.matrix(dummy), min.mean=0, use.ranks=FALSE, BSPARAM=ExactParam())
    expect_identical(out, ref)
})
