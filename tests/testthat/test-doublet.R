...
# Using higher counts here, for some variety.
set.seed(99000077)
ngenes <- 100
mu1 <- 2^rexp(ngenes) * 5
...
test_that("doubletRecovery works as expected", {
    ...
    ref <- doubletRecovery(sce, known.doublets, samples=c(1, 1, 1))
    ...
    # Responds to 'k'.
    alt <- doubletRecovery(sce, known.doublets, samples=c(1, 1, 1), k=20)
    expect_false(identical(ref, alt))

    # Responds to 'samples'
    alt <- doubletRecovery(sce, known.doublets, samples=c(1, 2, 3))
    expect_false(identical(ref, alt))

    # subset.row has the intended effect
    sub <- doubletRecovery(assay(sce), known.doublets, samples=c(1, 1, 1), subset.row=1:50)
    alt <- doubletRecovery(assay(sce)[1:50,], known.doublets, samples=c(1, 1, 1))
    expect_identical(sub, alt)
})

set.seed(99000008)
test_that("doubletRecovery gives the correct results on the toy example", {
    known.doublets <- clusters==3 & 1:2==1 # alternating doublets.
    ref <- doubletRecovery(sce, known.doublets, samples=c(1, 1), k=10)
...
set.seed(99000008)
test_that("doubletRecovery works for other inputs", {
    known.doublets <- clusters==3 & rbinom(length(clusters), 1, 0.5)==0
    ref <- doubletRecovery(logcounts(sce), known.doublets, samples=c(1, 1, 1))
    alt <- doubletRecovery(sce, known.doublets, samples=c(1, 1, 1))
    expect_identical(ref, alt)

    # Works for transposition
    alt <- doubletRecovery(t(logcounts(sce)), known.doublets, samples=c(1, 1, 1), transposed=TRUE)
    expect_identical(ref, alt)

    # Works by stuffing values in reduced dims.
    reducedDim(sce, "pretend") <- t(logcounts(sce))
    alt <- doubletRecovery(sce, known.doublets, samples=c(1, 1, 1), use.dimred="pretend")
    expect_identical(ref, alt)
})
